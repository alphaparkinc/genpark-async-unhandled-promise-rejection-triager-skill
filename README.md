# genpark-async-unhandled-promise-rejection-triager-skill
...
**genpark-async-unhandled-promise-rejection-triager-skill** is a deterministic, zero-dependency Python skill engineered for autonomous AI code reviews, AST-level taint flow tracing, breaking API change audits, and test flakiness mitigation.

> **Executive Capability**: Asynchronous control flow auditor identifying unawaited promises & race conditions (ESLint)
...
result = client.triage_async_concurrency_hazards()
print(result)
```
...
```json
{
  "mcpServers": {
    "genpark-async-unhandled-promise-rejection-triager-skill": {
      "command": "python",
      "args": ["/path/to/genpark-async-unhandled-promise-rejection-triager-skill/mcp_server.py"]
    }
  }
}
```
